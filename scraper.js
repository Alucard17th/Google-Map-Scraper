...
const phoneSelectors = [
  ...
  exportToJson(businesses, filename = 'results.json') {
    if (!businesses || businesses.length === 0) {
      console.log("No data to export");
      return false;
    }
...
async function main() {
  ...
  try {
    ...
    const exportChoice = await askWithValidation(
      '📊 Export format (csv/excel/json/all) [default: all]: ',
      (input) => ['csv', 'excel', 'json', 'all', ''].includes(input.toLowerCase()),
      'Please enter csv, excel, json, or all'
    );
    ...
    const exportFormat = exportChoice.toLowerCase() || 'all';
    ...
    if (results.length > 0) {
      ...
      const jsonFile = `${outputDir}/businesses_${timestamp}.json`;
      
      console.log('\n' + '� EXPORTING DATA...'.padStart(40, '=').padEnd(60, '='));
      
      let exportCount = 0;
      if (['csv', 'all'].includes(exportFormat)) {
        if (scraper.exportToCsv(results, csvFile)) exportCount++;
      }
      if (['excel', 'all'].includes(exportFormat)) {
        if (scraper.exportToXlsx(results, xlsxFile)) exportCount++;
      }
      if (['json', 'all'].includes(exportFormat)) {
        if (scraper.exportToJson(results, jsonFile)) exportCount++;
      }
